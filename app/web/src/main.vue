      ...
      <section class="hero is-info">
        <div class="hero-body">
          <div class="container">
            ...
            <h2 class="subtitle">国立国会図書館の提供するデジタル化資料のOCR全文テキストデータを利用したNgram Viewer(著作権保護期間が満了した図書資料約28万点が対象)</h2>
          </div>
        </div>
      </section>
        ...
        <div class="container">
          <div class="notification" v-show="deleteNot">
            ...
            <div class="content">
              ...
              <br />2022年5月現在の対象は、国立国会図書館デジタルコレクションでインターネット公開されている資料のうち、著作権保護期間が満了した図書資料約28万点（次世代デジタルライブラリーの全文検索と同じ資料群）のOCRテキストデータから集計した、約8.3億種類の単語及びフレーズです。
                  ...
                  <p>
                    <table class="table is-half">
                      ...
                      <tbody>
                        <tr>
                          <td>.</td>
                          <td><a href="./?keyword=修.者" target="_blank" rel="noopener noreferrer">修.者</a></td>
                          <td>任意の1文字を表す</td>
                        </tr>
                        <tr>
                          <td>*</td>
                          <td><a href="./?keyword=日*精進" target="_blank" rel="noopener noreferrer">日*精進</a></td>
                          <td>直前の表現が0個以上あることを表す</td>
                        </tr>
                        <tr>
                          <td>+</td>
                          <td><a href="./?keyword=郵便局%2B長" target="_blank" rel="noopener noreferrer">'郵便局+長'</a></td>
                          <td>直前の表現が1個以上あることを表す</td>
                        </tr>
                        <tr>
                          <td>?</td>
                          <td><a href="./?keyword=巡査部?長" target="_blank" rel="noopener noreferrer">巡査部?長</a></td>
                          <td>直前の表現が0個か1個あることを表す</td>
                        </tr>
                        <tr>
                          <td>{,}</td>
                          <td><a href="./?keyword=私を.%7B2,3%7Dにする" target="_blank" rel="noopener noreferrer">私を.{2,3}にする</a></td>
                          <td>直前の文字の繰り返し回数の範囲を指定する。例1:{2,4}:2回以上4回以下 例2:{3,}:3回以上 例3:{,5}:5回以下</td>
                        </tr>
                        <tr>
                          <td>[…]</td>
                          <td><a href="./?keyword=大正%5Bーア-ン%5D%7B6,%7D" target="_blank" rel="noopener noreferrer">大正[ーア-ン]{6,}</a></td>
                          <td>ブラケット内の1文字を表す。「-」で範囲を、「^」で否定を表す。例1[abc]:a,b,cのうち1文字 例2[ア-ン]：アからンまでの1文字（カタカナいずれか1文字） 例3[^ア-ン]：アからンまで以外の1文字（カタカナ以外の1文字）</td>
                        </tr>
                        <tr>
                          <td>(…)</td>
                          <td><a href="./?keyword=春の海ひねもす(のたり)*" target="_blank" rel="noopener noreferrer">春の海ひねもす(のたり)*</a></td>
                          <td>かっこで囲んだ範囲のグループを形成し、単一の文字として扱う。（他の演算子と組み合わせて用いる）</td>
                        </tr>
                        <tr>
                          <td>|</td>
                          <td><a href="./?keyword=ご%28機嫌%7Cきげん%29よう" target="_blank" rel="noopener noreferrer">ご(機嫌|きげん)よう</a></td>
              ...
              <br />【注意3】NDL Ngram Viewerの可視化対象は次世代デジタルライブラリーの検索対象とほぼ同じですが、厳密に同じデータを参照するものではありません。クエリによっては差異が発生することがあります。
              <br />【注意4】負荷対策のため、接頭及び接尾が両方とも正規表現で記述されたクエリは受け付けません。（受け付けない例：「.*テスト.*」）
              <br /><b>【注意5】明治期～昭和戦前期以外の期間に出版された資料については、点数が少ないことにご留意ください。</b>
              <br />
          ...
          <div v-if="dataload==true&&pagefrom==0">
            ...
            <div class="columns">
              <div class="column is-two-thirds">
                <b-slider class="is-size-7-touch" :min="1801" :max="2000" v-model="yearRange" type="is-danger" tooltip-always></b-slider>
              </div>
          ...
          <div v-if="result!=null" >
            ...
            <a href="https://lab.ndl.go.jp/dataset/ngramviewer/ngramsummary.tsv"><b-icon icon="download"></b-icon><span>出版年代ごとの総対象ngram数の情報をダウンロード</span></a>
            ...
            <table class="table is-fullwidth">
              ...
              <tbody>
                <tr v-for="(vs,i) in result.list" :key="i">
                  ...
                  <td>{{vs.count}}</td>
                  <td><a :href="'https://lab.ndl.go.jp/dl/fulltext?searchfield=contentonly&keyword='+vs.ngramkeyword" target="_blank" rel="noopener noreferrer">次世代デジタルライブラリーで検索</a></td>
                </tr>
              </tbody>
            </table>
          </div>
...
import { search,downloadurl,SearchResult} from "./service/search-service";
...
export default class extends Vue {
  ...
  async search() {
    if(this.input){
        ...
        if(this.groupstr!=null){
          var pushobj:any={query: { keyword: this.input,size:this.pagesize,from:this.pagefrom,groupstr:this.groupstr}};
        }else{
          var pushobj:any={query: { keyword: this.input,size:this.pagesize,from:this.pagefrom}};
        }
        this.$router.push(pushobj).catch(()=>{});
        try {
          this.result = (await search(encodeURIComponent(this.input),this.pagesize,this.pagefrom,this.groupstr)).data;
          this.fillData ();
          this.dataload=true;
        }catch{
  ...
  download(){
    return downloadurl(encodeURIComponent(this.input),this.groupstr);
  }
  searchbutton(){
    if(this.input!=null){
      this.pagefrom=0;
